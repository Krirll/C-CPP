...
void reportEvent(const TCHAR* const message) {
    ...
    if (handleEventLog != NULL) {
        LPCTSTR strings[1];
        strings[0] = message;
        ReportEvent(handleEventLog, EVENTLOG_ERROR_TYPE, 0, (DWORD)0xC0020001L, NULL, 1, 0, strings, NULL);
...
DWORD WINAPI worker(LPVOID param) {
    ...
    do {
        enum command cmd;
        res = recv(params->real_socket_file, (char*)&cmd, sizeof(cmd), 0); //для чтения данных из сокета
        if (res == SOCKET_ERROR) PRINTFd("recv #1 error: %d\n", WSAGetLastError())
        else if (res != 0) {
            if (res != sizeof(cmd)) {
                ...
                case WriteDouble: {
                    //добавление числа
                    int res2 = recv(params->real_socket_file, (char*)&number, sizeof(double), 0);
                    ...
                    if (res > 0) {
                        if (number < 0) {
                            ...
                            *params->lastNumber = (*params->lastNumber + 1) % MAXCOUNT;
                            if (*(params->count) == MAXCOUNT) {
                                *params->firstNumber = (*params->firstNumber + 1) % MAXCOUNT;
                            }
                            else {
                                (*(params->count))++;
                            }
                ...
                case GetAVG: {
                    ...
                    EnterCriticalSection(params->criticalSection);
                    double sum = 0;
                    for (int i = 0; i < *(params->count); i++, current = (current + 1) % MAXCOUNT) sum += params->numbers[current];
                    double avg = sum / *(params->count);
                    LeaveCriticalSection(params->criticalSection);
                    res2 = send(params->real_socket_file, (char*)&avg, sizeof(double), 0);
                    if (res2 == SOCKET_ERROR) {
                ...
                case Clean: {
                    ...
                    EnterCriticalSection(params->criticalSection);
                    for (int i = 0; i < *(params->count); i++, current = (current + 1) % MAXCOUNT) params->numbers[current] = 0;
                    *params->count = 0;
                    *params->lastNumber = -1;
                ...
                case GetSampleVariance: {
                    ...
                    for (int i = 0; i < *(params->count); i++) sum += params->numbers[i];
                    ...
                    LeaveCriticalSection(params->criticalSection);
                    res2 = send(params->real_socket_file, (char*)&disp, sizeof(double), 0);
                    if (res2 == SOCKET_ERROR) {
                        PRINTFd("send #1 error: %d\n", WSAGetLastError())
                            res = -1;
                    }
    ...
    if (closesocket(params->real_socket_file) == SOCKET_ERROR) PRINTFd("closesocket2 :%d\n", GetLastError())
        free(params);
    return 0;
}
...
DWORD WINAPI threadMainLoop(LPVOID p) {
    ...
    do {
        int real_socket_file = accept(prm->real_socket_file, NULL, NULL); //для принятия связи на сокет
        ...
        if (real_socket_file == INVALID_SOCKET) PRINTFd("accept error: %d\n", WSAGetLastError())
            ...
            if (params == NULL) {
                ...
                if (thread == NULL) PRINTF("error create thread\n");
                else CloseHandle(thread);
            }
            //free(params);
        } //else accept
    } while (true);
    res = closesocket(prm->real_socket_file); //проверка закрытия сокета
...
void WINAPI serviceProc(DWORD dwNumServicesArgs, LPTSTR* lpServiceArgVectors) {
    SERVICE_STATUS_HANDLE serviceStatusHandler = RegisterServiceCtrlHandler(_T("holder"), handler);
    if (serviceStatusHandler == 0) {
        ...
        if (!SetServiceStatus(serviceStatusHandler, &serviceStatus)) {
            ...
            if (!SetServiceStatus(serviceStatusHandler, &serviceStatus)) {
                ...
                if (res != 0) { //проверка инициализации
                    ...
                    if (socket_file == INVALID_SOCKET) {
                        ...
                        if (res == SOCKET_ERROR) PRINTFd("bind error: %d\n", WSAGetLastError())//
                        else {
                            ...
                            if (res == SOCKET_ERROR) PRINTFd("listen error: %d\n", WSAGetLastError())
                            else {
                                CRITICAL_SECTION criticalSection;
                                ...
                                if (params == NULL) {
                                    ...
                                    mainThread = CreateThread(NULL, 0, threadMainLoop, params, 0, NULL);
                                    if (mainThread == NULL) PRINTFd("error create thread: %d\n", GetLastError())
                                    else {
                                        while (!exiting);
